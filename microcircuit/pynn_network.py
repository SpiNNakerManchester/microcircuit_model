...
class PyNNNetwork(object):

    def __init__(self):
        pass

    @staticmethod
    def setup(sim_params, network_params):
        # if parallel_safe=False, PyNN offsets the seeds by 1 for each rank
        script_rng = NumpyRNG(seed=network_params.pyseed,
                              parallel_safe=network_params.parallel_safe)

        # Compute DC input before scaling
        if network_params.input_type == 'DC':
            dc_amp = {}
            for target_layer in network_params.layers:
                dc_amp[target_layer] = {}
                for target_pop in network_params.pops:
                    if network_params.neuron_model == 'iaf_psc_exp_ps':
                        tau_syn = network_params.neuron_params['tau_syn_ex']
                    else:
                        tau_syn = network_params.neuron_params['tau_syn_E']
                    dc_amp[target_layer][target_pop] = \
                        network_params.bg_rate * \
                        network_params.k_ext[target_layer][target_pop] * \
                        network_params.w_mean * tau_syn / 1000.0
        else:
            dc_amp = {
                'L23': {'E': 0., 'I': 0.},
                'L4': {'E': 0., 'I': 0.},
        ...
        # Network scaling
        if K_scaling != 1:
            self.w, self.w_ext, self.dc_amp = adjust_w_and_ext_to_K(K_full,
                                                                    K_scaling,
                                                                    self.w,
                                                                    self.dc_amp)
        else:
        ...
        if simulator == 'nest':
            if record_corr:
                ...
                sim.nest.SetDefaults('correlomatrix_detector',
                                     {'delta_tau': 0.5})
                self.corr_detector = sim.nest.Create('correlomatrix_detector')
                sim.nest.SetStatus(self.corr_detector,
        ...
        if sim.rank() == 0:
            ...
            for layer in sorted(layers):
                for pop in sorted(pops):
                    ...
                    if simulator == 'nest':
                        if not record_fraction and n_record > int(
                                round(N_full[layer][pop] * N_scaling)):
                            print(
                            'Note that requested number of neurons to record exceeds ', \
                ...
                if simulator == 'nest':
                    if record_corr:
                        ...
                        sim.nest.SetDefaults('static_synapse',
                                             {'receptor_type': index})
                        sim.nest.ConvergentConnect(list(this_pop.all_cells),
                                                   self.corr_detector)
            ...
            for target_pop in sorted(pops):
                ...
                if input_type == 'poisson':
                    ...
                    if simulator == 'nest':
                        ...
                        if sim.rank() == 0:
                            print(
                            'connecting Poisson generator to', target_layer,
                            target_pop, ' via SLI')
                        sim.nest.sli_run(
                            '/poisson_generator Create /poisson_generator_e Set poisson_generator_e << /rate ' \
                            + str(rate) + ' >> SetStatus')
                        sim.nest.sli_run("poisson_generator_e " + str(
                            list(this_target_pop.all_cells)).replace(',', '') \
                                         + " [" + str(
                            1000 * w_ext) + "] [" + str(
                            d_mean['E']) + "] DivergentConnect")
                    else:
                        if sim.rank() == 0:
                            print(
                            'connecting Poisson generators to', target_layer,
                            target_pop)
                        ...
                        sim.Projection(poisson_generator, this_target_pop, conn,
                ...
                if thalamic_input:
                    ...
                    if sim.rank() == 0:
                        print(
                        'creating thalamic connections to ' + target_layer + target_pop)
                    C_thal = thal_params['C'][target_layer][target_pop]
                    n_target = N_full[target_layer][target_pop]
                    K_thal = round(np.log(1 - C_thal) / np.log(
                        (n_target * thal_params['n_thal'] - 1.) / \
                        (n_target * thal_params[
                            'n_thal']))) / n_target * K_scaling
                    if conn_routine == 'fixed_total_number':
                        if simulator == 'spiNNaker':
                            FixedTotalNumberConnect_SpiNNaker(sim,
                            ...
                            FixedTotalNumberConnect_NEST(sim,
                                                         self.thalamic_population, \
                                                         this_target_pop,
                                                         K_thal, w_ext,
                                                         w_rel * w_ext, \
                        ...
                        FromListConnect(sim, self.thalamic_population,
                                        this_target_pop, 'excitatory',
                                        self.base_neuron_ids)
                        ...
                        if conn_routine == 'fixed_total_number':
                            if simulator == 'spiNNaker':
                                FixedTotalNumberConnect_SpiNNaker( \
                                    ...
                                    self.K[target_index][source_index], weight,
                                    w_sd, \
                            ...
                            FromListConnect(sim, this_source_pop,
                                            this_target_pop, conn_type,
                                            self.base_neuron_ids)
