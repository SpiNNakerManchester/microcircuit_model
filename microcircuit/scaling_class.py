...
class Scaling(object):
    ...
    def get_indegrees(self, network_params):
        ...
        for target_layer in layers:
            ...
            for target_pop in pops:
                ...
                for source_layer in layers:
                    for source_pop in pops:
                        source_index = structure[source_layer][source_pop]
                        x1 += w[target_index][source_index] * \
                ...
                if input_type == 'poisson':
                    ...
                    I_ext[target_layer][target_pop] = 0.001 * neuron_params[
                        'tau_syn_E'] * \
                                                      ((1. - np.sqrt(
                                                          internal_scaling)) * x1 + \
                                                       (1. - np.sqrt(
                                                           external_scaling)) * x1_ext) + \
                                                      DC[target_layer][
                                                          target_pop]
                else:
                    w_ext_new = np.nan
    ...
    def adjust_w_and_g_to_K(K_full, K_scaling, w):
        ...
        w_new = {}
        w_m_matrix = np.zeros(
            [n_layers * n_pops_per_layer, n_layers * n_pops_per_layer])
        for target_layer in layers:
            g_new[target_layer] = {}
            w_new[target_layer] = {}
            for target_pop in pops:
                ...
                for source_layer in layers:
                    for source_pop in pops:
                        source_index = structure[source_layer][source_pop]
                        x0[source_pop] += K_full[target_index][source_index] * \
                                          full_mean_rates[source_layer][
                                              source_pop]
                        x1[source_pop] += w[target_index][source_index] * \
                                          K_full[target_index][source_index] * \
                                          full_mean_rates[source_layer][
                                              source_pop]
                        x2[source_pop] += (w[target_index][source_index]) ** 2 * \
                                          K_full[target_index][source_index] * \
                                          full_mean_rates[source_layer][
                                              source_pop]
                ...
                N_min = (x1['E'] + x1['I']) ** 2 / (
                (x0['E'] + x0['I']) * (x2['E'] + x2['I']))
                w_new[target_layer][target_pop] = (np.sqrt(x0['E']) * (
                x1['E'] + x1['I']) \
                                                   + np.sqrt(
                    x0['I'] * K_scaling * (x0['E'] + x0['I']) * (
                    x2['E'] + x2['I']) \
                    - x0['I'] * (x1['E'] + x1['I']) ** 2)) / (
                                                  K_scaling * np.sqrt(
                                                      x0['E']) * (
                                                  x0['E'] + x0['I']))
                g_new[target_layer][target_pop] = (x1['E'] + x1['I'] -
                                                   w_new[target_layer][
                                                       target_pop] \
                                                   * K_scaling * x0['E']) / (
                                                  w_new[target_layer][
                                                      target_pop] * K_scaling *
                                                  x0['I'])
            ...
            for target_pop in pops:
                ...
                for source_layer in layers:
                    for source_pop in pops:
                        ...
                        x2_new[source_pop] += (w_m_matrix[target_index][
                                                   source_index]) ** 2 * K_scaling * \
                                              K_full[target_index][
                                                  source_index] * \
                                              full_mean_rates[source_layer][
                                                  source_pop]
